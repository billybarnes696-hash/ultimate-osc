...
st.set_page_config(
    page_title="RSP Ultimate Oscillator",
    page_icon="📈",
    layout="wide",
...
def build_oscillator(price: pd.Series, bundle: Optional[Dict[str, pd.Series]], cfg: AppConfig) -> pd.DataFrame:
    ...
    source_map = {
        "SPXA50R": bundle.get("_spxa50r"),
        "BPSPX": bundle.get("_Bpspx"),
        "BPNYA": bundle.get("_Bpnya"),
        "NYMO": bundle.get("_nymo"),
        "NYSI": bundle.get("_nySI"),
        "NYAD": bundle.get("_nyad"),
        "TRIN": bundle.get("_trin"),
        "CPCE": bundle.get("_cpce") if bundle.get("_cpce") is not None else bundle.get("_cpc"),
        "OEXA50R": bundle.get("_oexa50r"),
        "SPXADP": bundle.get("_spxadp"),
    }
    transformed = {}
...
def run_backtest(df: pd.DataFrame, cfg: AppConfig) -> Dict[str, object]:
    ...
    for i in range(1, len(work)):
        ...
        if not in_position and enter_long:
            ...
            entry_osc = curr_osc
            bars_held = 0
            strategy_returns.iloc[i] -= 0.0003
...
@st.cache_data(show_spinner=False)
def build_model(zip_bytes, snapshot_bytes, ticker, period, lookback, smooth_price, smooth_breadth, breadth_weight, stop_loss_pct, target_vol, max_hold_bars):
...
st.caption("Built for your StockCharts breadth zip + daily snapshot workflow.")
...
st.info("This version is tailored to your uploaded breadth zip and snapshot file format. Nothing heavy runs until you click **Build Ultimate Oscillator**.")
...
try:
    ...
    with st.spinner("Building model..."):
        osc_df, backtest, summary, bundle, snapshot_map = build_model(
            ...
            lookback=lookback, smooth_price=smooth_price, smooth_breadth=smooth_breadth,
            breadth_weight=breadth_weight, stop_loss_pct=stop_loss_pct, target_vol=target_vol, max_hold_bars=max_hold_bars,
        )
    if osc_df.empty:
    ...
    with tab4:
        ...
        st.markdown("""
**Expected files**
- Historical breadth ZIP like your uploaded `breadth.zip`
- Daily snapshot CSV like your uploaded `SC (22).csv`
